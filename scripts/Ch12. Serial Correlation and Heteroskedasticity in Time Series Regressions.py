...
# The coefficient on the lagged residual (resid_s_lag1) is 0.5741, and it is highly
# statistically significant (p-value < 0.0001). This provides strong evidence of positive
...
# The coefficient on the lagged residual (resid_ea_lag1) is 0.0551, which is much smaller
...
# The F-statistic is large (22.88) and the p-value is very small (< 0.0001).
...
fstat_manual = ftest_manual.statistic[0][0]  # Extract F-stat value
...
# Interpretation:
# - The estimated AR(1) coefficient rho is 0.8358, indicating strong positive serial correlation, consistent with the BG test.
...
# The coefficient on log(mincov) is -0.1538 and highly significant (p < 0.0001),
...
# - The HAC standard error for log(mincov) is 0.0696, which is considerably larger than the standard OLS SE (0.0440).
...
# - The coefficient on the lagged squared residual (resid_sq_lag1) is 0.1405.
# - This coefficient is positive and highly statistically significant (p-value < 0.0001).
